...
describe('Variants', () => {
  ...
  describe('getStatsLabels', () => {
    ...
    it.only('passes the correct variant to ibl', async () => {
      const expectedExperimentVariant = {
        iplxp_pb01_pikachu: 'thunderbolt'
      };

      await requestHomepage()
        .set('x-unique-id', 'some-awesome-id')
        .set('x-experiment-iplxp_pb01_pikachu', 'thunderbolt')
        .expect(200);

      sandbox.assert.calledWith(
        iblGraph.graphQLById,
        sinon.match.any,
        sinon.match.any,
        sinon.match.any,
        expectedExperimentVariant
      );
    });
