...
import iblEpisodesResponseFixture from '../fixtures/bar.json';
...
describe.only('Onward Journeys', () => {
  ...
  it('calls fetch with the correct arguments when fetching episode when clicking a related episode', async () => {
    ...
    window.fetch.resolves({
      json: sandbox.stub().resolves(iblEpisodesResponseFixture)
    });
  ...
  it.only('calls fetch with correct arguments when fetching related episodes when clicking a related episode', async () => {
    ...
    jsonStub.onCall(0).returns(() => { return Promise.resolve(iblEpisodesResponseFixture); });
    jsonStub.onCall(1).returns(() => { return Promise.resolve(iblRelatedEpisodesResponseFixture); });
    jsonStub.onCall(2).returns(() => { return Promise.resolve(iblRelatedEpisodesResponseFixture); });
