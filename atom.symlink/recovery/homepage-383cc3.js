...
function getGraphQLClient({ identity, userInDefaultU16Variant }) {
  ...
  return {
    graphQLById: (queryId, variables = {}) => {
      ...
      return iblGraph.graphQLById(queryId, variablesWithUser, auth);
    }
  };
}
...
async function handleGraphQLHomepage({ req, res, identity, lang, translator, userPageBuckets, bucketsFromRequestHeaders, pageExperimentsConfig }) {
  const { headers, cookiePolicy, uniqueId, region: { userAttributes } } = req;
  ...
  const ibl = getGraphQLClient({ identity, userInDefaultU16Variant });
