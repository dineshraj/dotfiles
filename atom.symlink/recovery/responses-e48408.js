...
const iblExperimentationClient = require('../../../lib/clients/iblExperimentation');
const idClient = require('../../../lib/clients/id');
const iplayerExperimentation = require('../../../lib/clients/experimentation');
...
const experimentationContextFixture = require('../../fixtures/experimentationContext');
...
describe('GET /iplayer/episode', () => {
  beforeEach(async () => {
    ...
    sandbox.stub(iblExperimentationClient, 'getExperimentsContext').resolves(experimentationContextFixture);
    ...
    await iplayerExperimentation.start();
  });
  ...
  it('passes the correct experimentation variants to ibl', async () => {
    const expectedExperimentVariant = {
      iplxp_pb01_pikachu: 'thunderbolt'
    };
    await callPriorityKind();
    sinon.assert.calledWith(iblClient.getEpisodes, sinon.match.any, sinon.match.any, expectedExperimentVariant);
    sinon.assert.calledWith(iblClient.getNextEpisode, sinon.match.any, sinon.match.any, expectedExperimentVariant);
    sinon.assert.calledWith(iblClient.getEpisodePrerolls, sinon.match.any, expectedExperimentVariant);
    sinon.assert.calledWith(iblClient.getProgrammeEpisodes, sinon.match.any, sinon.match.any, expectedExperimentVariant);
  });
