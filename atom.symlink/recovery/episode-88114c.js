...
describe('Episode Util', () => {
  ...
  describe('.getdownloadURIs(episodeFixture)', () => {
    ...
    it.only('does not overwrite downloadURIs when presented with two non-accessible version', () => {
      const expectedURI = 'bbc-ipd:download/b084ftll/b084ft6s/sd/standard/UGFub3JhbWEgLSBUZWVuYWdlIFByaXNvbiBBYnVzZSBFeHBvc2Vk/p02544td/UGFu';

      const episodeWithTwoNonAccessibleVersions = _.merge(
        {},
        episodeFixture,
        {
          versions: [
      ...
      const downloadURIs = episodeUtil.getdownloadURIs(episodeWithTwoNonAccessibleVersions);

      assert.equal(downloadURIs.SD, expectedURI);
    });

    it.only('uses another available non-accessible version if no original version exists', () => {
